...
if (isset($_GET['print']))
{
	...
	if ($_GET['print'] == 'week')
	{
		if (isset($_GET['week']))
		{
			...
			echo "<tr>";
			echo "  <td valign='top'>\n";
			$AUF->printItemsDay($montag->format('Y-m-d'));
			echo "  </td>\n";
			echo "  <td valign='top'>\n";
			$AUF->printItemsDay($dienstag->format('Y-m-d'));
			echo "  </td>\n";
			echo "  <td valign='top'>\n";
			$AUF->printItemsDay($mittwoch->format('Y-m-d'));
			echo "  </td>\n";
			echo "  <td valign='top'>\n";
			$AUF->printItemsDay($donnerstag->format('Y-m-d'));
			echo "  </td>\n";
			echo "  <td valign='top'>\n";
			$AUF->printItemsDay($freitag->format('Y-m-d'));
			echo "  </td>\n";
			echo "</tr>";
			echo "</table>";
			echo "<hr />\n";
			...
			$AUF->printItemsWeek($woche->format('Y-m-d'));
			echo "<br clear='all' />\n";
			echo "<h1>Woche ".$montag->format('W')." auf Abruf:</h1>";
			$AUF->printItemsWeek($abruf->format('Y-m-d'));
			echo "</div>\n";
		}
	}
		...
		if (is_numeric($_GET['anz']))
		{
			...
			for ($w = 1; $w <= $weeks; $w++)
			{
				...
				for ($i = 1; $i <= 6; $i++)
				{
					...
					echo "<ul>\n";
					$AUF->printItemsDay($tag->format('Y-m-d'));
					echo "</ul>\n";
