...
class TRA
{
	...
	{
		$sql 		= "SELECT * FROM `tra_items` WHERE `erledigt` IS NULL AND `datum` = '".$tag."' ORDER BY `prio`, tra_items.auftrag ASC";
	...
	{
		$sql 		= "SELECT * FROM `tra_items` WHERE `erledigt` IS NOT NULL AND `datum` = '".$tag."' ORDER BY `prio`, tra_items.auftrag ASC";
		$result 	= $this->DB->query($sql);
		foreach ($result as $item)
		{
	...
	{
		$sql 		= "SELECT * FROM `tra_items` WHERE `erledigt` IS NULL AND datum IS NULL ORDER BY `prio`, tra_items.auftrag ASC";
		$result 	= $this->DB->query($sql);
		foreach ($result as $item)
		{
			echo "<div id='".$item['id']."' class='drag t".$item['prio']."'><b>".$item['prio']." - ".$item['auftrag']."</b><div class=\"toggle\" style=\"display: none;\">".nl2br($item['bemerkung'])."</div></div>";
	...
	{
		$sql 		= "SELECT * FROM `tra_items` WHERE `erledigt` IS NOT NULL ORDER BY `datum` DESC";
		$result 	= $this->DB->query($sql);
		echo "<ul>\n";
		foreach ($result as $item)
		{
	...
	function suchResultate($suchbegriff)
	{
		...
		echo "<div style=\"width:350px;float:left;display:block\">\n";
				echo "<h4>Transportverwaltung</h4>\n";
		...
		foreach ($result as $item)
		{
			...
			echo "	<li><p><a href=\"./tra_edit.php?id=".$item['id']."\">".$item['prio']." - ".$item['auftrag']."</a><br />Geplant am: ".$datum."<br /> <b>Erledigt am: ".$erledigt."</b><br />".nl2br($item['bemerkung'])."</p></li>\n";
		}
		echo "</ul>\n";
		echo "</div>\n";
	}
