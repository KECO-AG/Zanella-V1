...
window.onload = function () {
	...
	rd.myhandler_dropped = function () {
		...
		// save table content
		//var wback = 2;
		send_request('./scripts/tra-save.php?wback=' + wback + '&p=' + rd.obj.id + '_' + pos.join('_'));
	...
	rd.myhandler_deleted = function () {
		...
		var pos = rd.get_position(),
			...
			col = pos[5];
		// delete element
		send_request('./scripts/tra-delete.php?p=' + rd.obj.id + '_' + row + '_' + col);
	};
	rd.myhandler_dblclicked = function () {
		var id = rd.obj.id;
		$.ajax({
			url: "./scripts/tra-edit.php?action=ajax-get-data&id=",
			type: "POST",
			data: ({id : rd.obj.id}),
		...
		function (){
			var id = rd.obj.id;
			window.location = "/tra_edit.php?id="+id;
		}
		**/
	};
};
