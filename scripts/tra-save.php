...
if ($tbl1 == 1)
{
	...
	$werte = $p;
	$sql = "UPDATE tra_items SET tra_items.datum = '".$datum."' WHERE tra_items.id = '".$sub_id."'";
	$DB->query($sql);
}
if ($tbl1 == 0 && $col1 == 2)
{
	...
	$sql = "UPDATE tra_items SET tra_items.erledigt = '".$timestamp->format('Y-m-d G:i:s')."' WHERE tra_items.id = '".$sub_id."'";
	$DB->query($sql);
}
