...
switch ($_GET['action'])
{
	case 'neuesPaket':
		...
		else
		{
			...
			$sql = "INSERT INTO rhl_lager (holzart,qualitaet,trocknung,paket,preis,dicke,breite,laenge,stk,date,deleted) VALUES ('".$holzart."', '".$qualitaet."', '".$trocknung."', '".$paketnr."', '".$preis."', '".$dicke."', '".$breite."', '".$laenge."', '".$stk."','".$datum."', NULL)";
			$DB->query($sql);
			...
			(header("Location: ../rhl_newpack.php?message=2&lastITEM=".$id.""));
		}
		break;
	...
	case 'delete':
		...
		$sql = "UPDATE rhl_lager SET rhl_lager.deleted = '".$timestamp->format('Y-m-d G:i:s')."' WHERE id='".$id."'";
		$DB->query($sql);
		/*
